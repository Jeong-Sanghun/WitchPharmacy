...
public class BookResearchManager : ResearchManagerParent
{


    BookResearchDataWrapper dataWrapper;

    // Start is called before the first frame update
    protected override void Start()
    {
        base.Start();
        dataWrapper = gameManager.bookResearchDataWrapper;
        MakeButtonCanvas();
    }
    ...
    {
        //끝난 연구
        List<string> researchEndBook = researchSaveData.endBookResearchList;
        //제이슨 데이터
        List<BookResearchData> dataList = dataWrapper.bookResearchDataList;
        int nowButtonIndex = 0;
        for (int i = 0; i < dataList.Count; i++)
        {
            //끝났으면 안만들어줌.
            if (researchEndBook.Contains(dataList[i].fileName))
            {
                continue;
            }
            ...
            {
                if (!researchEndBook.Contains(dataList[i].neededResearchList[j]))
                {
                    contain = false;
                    break;
                }
            ...
            for (int j = 0; j < researchSaveData.progressingBookResearchList.Count; j++)
            {
                if (data.fileName == researchSaveData.progressingBookResearchList[j].fileName)
                {
                    research = researchSaveData.progressingBookResearchList[j];
                    break;
                }
            }
            if (research == null)
            ...
            else
            {
                ...
                {
                    researchSaveData.progressingBookResearchList.Remove(research);
                    researchSaveData.endBookResearchList.Add(research.fileName);
    ...
    {
        BookResearchData data = (BookResearchData)wholeButtonList[index].data;
        OneResearch research = null;
        for (int i = 0; i < researchSaveData.progressingBookResearchList.Count; i++)
        {
            if (data.fileName == researchSaveData.progressingBookResearchList[i].fileName)
            {
                research = researchSaveData.progressingBookResearchList[i];
                break;
            }
        }
        if (research == null)
        {
            ...
            researchSaveData.progressingBookResearchList.Add(research);
        }
        else
        {
            research.researchedTime += researchCount;
        ...
        if (research.researchedTime >= data.researchEndTime)
        {
            ...
            saveData.symptomBookList.Add(data.fileName + "+");
            saveData.symptomBookList.Add(data.fileName + "-");

            //int containingIndex = -1;
            //for (int i = 0; i < wholeButtonList.Count; i++)
            //{
            //    BookResearchData researchData = (BookResearchData)wholeButtonList[i].data; ;
