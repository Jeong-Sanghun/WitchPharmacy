...
public class OtherToolResearchManager : ResearchManagerParent
{
    OtherToolResearchDataWrapper dataWrapper;
    ...
    {
        //끝난 연구
        List<string> researchEndOtherTool = researchSaveData.endOtherToolResearchList;
        //제이슨 데이터
        List<OtherToolResearchData> dataList = dataWrapper.otherToolResearchDataList;
        ...
        {
            //끝났으면 안만들어줌.
            if (researchEndOtherTool.Contains(dataList[i].fileName))
            {
                continue;
            }
            ...
            {
                if (!researchEndOtherTool.Contains(dataList[i].neededResearchList[j]))
                {
                    contain = false;
                    break;
                }
            ...
            for (int j = 0; j < researchSaveData.progressingOtherToolResearchList.Count; j++)
            {
                if (data.fileName == researchSaveData.progressingOtherToolResearchList[j].fileName)
                {
                    research = researchSaveData.progressingOtherToolResearchList[j];
                    break;
                }
            }
            if (research == null)
            ...
            else
            {
                ...
                {
                    researchSaveData.progressingOtherToolResearchList.Remove(research);
                    researchSaveData.endOtherToolResearchList.Add(research.fileName);
    ...
    {
        OtherToolResearchData data = (OtherToolResearchData)wholeButtonList[index].data;
        OneResearch research = null;
        for (int i = 0; i < researchSaveData.progressingOtherToolResearchList.Count; i++)
        {
            if (data.fileName == researchSaveData.progressingOtherToolResearchList[i].fileName)
            {
                research = researchSaveData.progressingOtherToolResearchList[i];
                break;
            }
        }
        if (research == null)
        {
            ...
            researchSaveData.progressingOtherToolResearchList.Add(research);
        }
        else
        {
            research.researchedTime+= researchCount;
        ...
        if (research.researchedTime >= data.researchEndTime)
        {
            ...
            {
                OtherToolResearchData researchData = (OtherToolResearchData)wholeButtonList[i].data; ;
